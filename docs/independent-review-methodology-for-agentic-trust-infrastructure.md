...
4. **Cross-service trust-boundary analysis and adversarial second-implementer review** as first-class stages (§13.6, §13.7), motivated by case evidence that both surfaced defects invisible to component-level and consistency review (§11.1: FDI-ANT-002 and FDI-RES-008 uniquely sourced to Stage 6; FDI-ANT-010, FDI-CRY-006, and FDI-OPS-005 to Stage 7). *Strength: adaptation of threat modeling and conformance testing, with case-grounded motivation.*
...
- **One CRITICAL cross-service finding** — Anthill stores but never verifies `node_signature` — was assigned its CRITICAL severity by the trust-boundary analysis (the underlying defect had been raised earlier by the component review and ledger — §11.1; §17, Example C) and remains open at the index's baseline, with code citations (`anthill/server.js`) recorded in the index.
...
Two derivation principles were applied throughout. First, **a stage earns inclusion only if the corpus shows it catching something another stage missed** — this is why cross-service analysis (§13.6) and second-implementer review (§13.7) are separate stages rather than sub-activities of component review. The clearest cases are the two findings uniquely sourced to cross-service analysis (the sequence-counter suppression FDI-ANT-002 and the stale-window revocation freeze FDI-RES-008), each recorded in the disposition index with no other stage's document as a source, and the mirror protocol's non-existence, which surfaced only under an attempted deployment. The `node_signature` defect (FDI-ANT-001) is a subtler case worth stating precisely, because it is easy to overclaim (§11.1, §17 Example C): it was *not* first discovered at the cross-service stage — the Anthill component review had already raised it at P0 and the Anthill specification openly disclosed it as a deferred item. What cross-service attack-chain analysis uniquely contributed was the CRITICAL *severity*, by making the fabrication, suppression, and framing impacts concrete. The lesson is therefore about severity assignment, not detection: a finding can be known, disclosed, and deferred while no stage has yet recognized the severity it warrants. Second, **the method must be honest about what the corpus does not demonstrate**: several stages (notably the disposition and temporal-maintenance stage, §13.11) are motivated by the corpus's *failure* — reviews that became stale — rather than its success, and are therefore proposals to prevent an observed problem, not observed successes.
...
### 11.1 Stage Derivation Table

The table below makes §11's derivation claim auditable. For each of the fourteen stages (§13), it records the review move the stage generalizes, the corpus artifact(s) showing the move was actually performed at the case baseline, a representative finding or artifact the move produced, and — applying the first derivation principle — whether the corpus shows the move catching something no other stage caught. Finding IDs are the canonical FDI-* identifiers of `docs/finding-disposition-index-2026-06.md` (baseline `c999fdd`); cells marked [REPORTED] cite corpus statements not independently re-verified in the preparation of this table. The final column classifies each stage's provenance per the note following the table.

| Stage | Review move (the analytical action) | Corpus evidence it was performed | Representative finding/artifact it surfaced | Did it catch something another stage missed? | Classification |
|---|---|---|---|---|---|
| 0 Charter & scope | Fix scope, claims, access, independence category, and stop conditions before writing findings | No standalone charter exists; the move appears piecemeal in per-document role/scope/mode headers (disposition index header: reviewer role, mode "review-and-index", declared "what was not done" list, §1.4) and the ledger's AI-008 provenance distinction | The disposition index's explicit not-done list (tests not executed; live deployment not inspected) | No unique catch in corpus — included as ESTABLISHED prerequisite. Its absence as a formal artifact is itself visible: independence wording drifted unchecked into "external review" (FDI-RESR-002) | ESTABLISHED |
| 1 Immutable baseline | Pin commit, versions, release hashes, and test state so findings are re-derivable | Per-round pinned commits across the consistency series (r3 `a3384bd` → r6 `a504204`, Table 2); disposition index pinned at `c999fdd`; README v1 hash table | The baseline manifests themselves | No unique catch in corpus — included as ESTABLISHED prerequisite. Pinning is what made FDI-XST-007 (behavior changed in W0 at an unchanged implementation version; contradiction K8) detectable by later stages | ESTABLISHED |
| 2 Inventory & authority hierarchy | Enumerate every artifact; decide which wins on disagreement; log contradictions | Disposition index §1 step 1 (every finding-bearing document inventoried); `documentation-set-review-2026-06-11.md`; contradiction table (index §9, K1–K16) | FDI-DOC-002 (README "no HIGH/MEDIUM open" contradicted by a document two directories away, doc-set C5); FDI-DOC-005 (dangling pointers to absent steward reports) | Yes — FDI-DOC-002 and FDI-DOC-005 have only cross-artifact review as source; no spec-, code-, or boundary-focused stage reads across artifacts this way | ESTABLISHED |
| 3 Claim decomposition | Split prose claims into testable subclaims with named required evidence types | Performed embedded, not standalone: no claim-to-evidence matrix (Appendix D) exists in the corpus; the move is visible where claims are split (determinism MUST decomposed into wall-clock / per-process liveness / unpinned-arithmetic subcomponents — FDI-RES-003; claim-wording analysis, index §13) | FDI-RES-003 (unscoped determinism MUST not achievable as specified) | No unique catch in corpus — FDI-RES-003 was independently raised by arch-res S4, dillclaw-gap G-20, and spec-gap RES-12/13 (other stages). Stage 3's incremental value is the subject of validation study S7 (§23), i.e., untested | ESTABLISHED |
| 4 Spec/impl/test consistency | Trace every MUST/SHOULD to code and test; flag divergence and phantom controls | Six-round series (`spec-consistency-review-2026-06-09*.md`, `-2026-06-10-r6.md`); reconciled series total 8 HIGH / 21 MEDIUM (§10.2) | FDI-DOC-012 (phantom provisional-tier "weighting penalty" asserted in specs but implemented nowhere; r2-H-2) | Yes — FDI-DOC-012 is sourced only to consistency rounds (r2, r4, r5): a spec asserting a control no code implements is invisible to component review (which reads the code) and to boundary review | ESTABLISHED |
| 5 Component architecture review | Per-component STRIDE-style analysis of state, interfaces, crypto, persistence, failure handling | Three architecture reviews (`architecture-review-{registry,resolver,anthill}-2026-06-10.md`) | FDI-REG-001 (single authoritative instance, no HA); FDI-RES-004 (synchronous, unbounded trace writes — a public-node disk-fill DoS) | Yes — several canonical findings carry a component review as sole source: FDI-REG-001 (arch-reg S1), FDI-ANT-005 (arch-ant S3), FDI-RES-006 (arch-res S7), FDI-ANT-007 (arch-ant S7) | ESTABLISHED |
| 6 Cross-service trust-boundary | For each boundary, state what the consumer assumes about the producer's fields and whether it is enforced; build cross-service attack chains | `cross-service-trust-boundary-analysis-2026-06-10.md` (findings F-1…F-11) | FDI-ANT-001 escalated to CRITICAL (node_signature stored, never verified → forgeable attributable governance signals) | Yes, with a correction: FDI-ANT-002 (sequence-counter poisoning, F-4) and FDI-RES-008 (stale-window revocation freeze, F-7) are sourced only to this stage. For FDI-ANT-001 the index records earlier sightings (ledger AUDIT-AS-001/AI-005 2026-05-15; arch-ant S1; Anthill spec A.11 self-discloses it); unique to this stage was the CRITICAL severity via the forgery/suppression/framing attack chains, not first discovery | ADAPTED |
| 7 Adversarial implementer | Attempt deployment and second implementation from public documents only; log every blocker | Four gap reports: `dillclaw-deployment-`, `registry-mirror-deployment-`, `anthill-signal-emitter-gap-report-2026-06-10.md`, `spec-gap-report-2026-06-10.md` | FDI-REG-002 (mirror "no happy path at all"); FDI-XST-003 (spec insufficient for second implementation — 88 gaps, 9 blockers [REPORTED] count from the index) | Yes — FDI-ANT-010 (no wire protocol), FDI-CRY-006 (trust-root distribution), FDI-OPS-005 (missing configuration reference) are sourced only to gap reports. Caveat: mirror non-implementation was also flagged by arch-reg S2; unique here is the demonstrated impossibility (no sync protocol; re-signing import path) under attempted use | ADAPTED |
| 8 Existing-infrastructure comparison | Classify each novel subsystem against established alternatives: necessary, duplicative, better-delegated | `architecture-review-registry-vs-existing-infrastructure-2026-06-10.md`; `anthill-vs-observability-stack-2026-06-10.md`; `ans-v2-and-capability-standing-boundary-analysis-2026-06.md` | Reclassification of bespoke transparency, trust-root distribution, and mirror machinery as duplicative (Example H); contributes sources to FDI-CRY-004, FDI-GOV-001 | No unique defect catch in the finding index — its unique output is strategic (roadmap invalidation feeding Stage 13), not a defect class other stages missed. Included as ESTABLISHED build-vs-reuse analysis | ESTABLISHED |
| 9 Remediation wave | Fix findings in a bounded, scoped batch with acceptance criteria and captured evidence | W0 wave: `v2-tracker.md`; wave commits verified by the index (`74cad67`, `25670a4`, `a1a95d1`, `4447c00`, `8c87a85`, `d25bf7b`, `d1466c0`, `e523652`); index §10 wave assessment | Verified closures FDI-RES-002 (SSRF) and FDI-RES-009 (catalog truncation) | No unique catch — not a detection stage. Its observed failure (design-W0 vs shipped-W0 scope drift, contradiction K4) is precisely what Stages 10–11 exist to catch | ESTABLISHED |
| 10 Verification & regression | Re-derive every claimed closure from primary evidence at the new baseline; never accept commit messages or issue states | Disposition index §1 step 3 (independent verification against code and git history); r3 fix-verification round | FDI-DOC-008 / FDI-RES-001 (spec documents client conditional fetch; resolver contains zero ETag/304 logic, verified by grep); FDI-REG-004 (found CLOSED in code while the ledger still said OPEN, K10) | Yes — the ETag no-consumer divergence, the ledger-lags-code closure, and r3's two fix-induced defects (one MUST-level [REPORTED]) are visible only under re-derivation; r3 and r5 commit messages overclaimed closure, so trusting them would have recorded false closures | ESTABLISHED |
| 11 Disposition & temporal maintenance | Normalize raw findings to canonical records with controlled dispositions and last-verified baselines; banner historical documents; keep a stale-document register | `finding-disposition-index-2026-06.md` (~230 raw → 70 canonical [REPORTED] normalization ratio); stale-document index (index §8); status banners now present on historical reviews | FDI-DOC-001 (the corpus carried no disposition layer — the gap the index remediates); contradiction K11 (ledger F-003 vs trust-boundary F-3 ID collision, a false-reassurance risk surfaced by normalization) | Motivated by failure, not success: the stage exists because reviews went stale and misled (Example G). Executed once in the corpus; its preventive value is untested (validation study S5) | HYPOTHESIZED |
| 12 Claim-safety & readiness by profile | Gate each finding per deployment profile; mark each public claim supported / qualify / retire | Disposition index §4 (current gates), §12 (profile matrix), §13 (claim-safety assessment), §15 | The differentiated verdict (Profile A "viable now"; B, C, D "not viable" for distinct, named reasons); FDI-RESR-002 ("external review" wording overstates independence) | No unique defect catch — the claim-wording findings rest on evidence produced by earlier stages. Its unique output is the profile-differentiated verdict itself, which no single severity label expresses | ADAPTED |
| 13 Falsification & strategic decision | State falsifiable hypotheses and kill criteria; decide continue / narrow / integrate / retire | `strategic-evaluation-2026-06-12.md`; the ANS boundary analysis; the decision executed in `dillweed-v1-preservation-and-transition-plan.md` | FDI-GOV-003 (governance corpus institutionalizes bodies that do not exist); FDI-RESR-001 (zero external validation) | Yes — FDI-GOV-003 and FDI-RESR-001 are sourced only to the strategic evaluation; and the narrow-and-integrate recommendation demonstrates the method can recommend against its own subject (§20), an outcome no technical stage produces | ESTABLISHED |

**How to read the classification column.** ESTABLISHED means the stage is standard practice — baseline pinning, threat modeling, traceability review, fix verification, build-vs-reuse analysis — assembled into this workflow, with no novelty claimed beyond the assembly. ADAPTED means an established practice was modified specifically for agentic trust infrastructure: Stage 6 refocuses threat modeling on one service trusting another's unverified field; Stage 7 turns conformance/interoperability testing into a documents-only deployment and reimplementation attempt; Stage 12 converts readiness-level thinking into per-profile finding gates and claim wording. HYPOTHESIZED means the stage is motivated by an observed corpus failure rather than a demonstrated success and has not been validated: Stage 11's temporal-maintenance discipline was executed once and its preventive value awaits study S5. The tally — ten ESTABLISHED, three ADAPTED (6, 7, 12), one HYPOTHESIZED (11) — matches the contribution strengths already conceded in §7.2 and §21; the method's claim is the integration, not fourteen novelties.

**What the table does and does not establish.** The table establishes that each stage corresponds to a review move actually performed in the corpus at the pinned baseline, and that seven stages (2, 4, 5, 6, 7, 10, 13) each have at least one canonical finding for which the disposition index records no other stage's document as a source — the corpus-side basis for the first derivation principle of §11. It also establishes, by recording nulls, that the remaining stages are retained as prerequisites (0, 1), embedded moves (3), a remediation step rather than a detection step (9), or strategic-output stages (8) — not as unique detectors. It does not establish that these moves would yield unique catches on another project (validation study S1), that the source attributions in the disposition index are themselves complete (a finding recorded against one document may have been informally noticed earlier), or that "sole source in the index" implies "undetectable by other stages" — it means only that, in this corpus, no other stage's document recorded it. Single-case limits (§10.3) apply to every row.
...
**Exit criteria.** Every boundary characterized; each "producer field" classified as verified/unverified by the consumer. *Case grounding:* this stage escalated the unverified `node_signature` defect to CRITICAL by constructing the forgery/suppression/framing attack chains — a severity assignment that the earlier component and consistency reviews, which had already noted the defect, did not make — and in the same pass produced two findings sourced to no other stage (FDI-ANT-002, FDI-RES-008), the paper's evidence that Stage 6 is not reducible to Stage 5 (§17, Example C; §11.1).
...
- **Reviewer/scope anchoring.** The series scope was *specification consistency*. It converged. Yet the single CRITICAL of the entire corpus — the unverified `node_signature` — was **not** in scope: the defect was already tracked at component-level severity elsewhere in the corpus, and it was escalated to CRITICAL only later by the cross-service trust-boundary analysis (§11.1; §17, Example C). A converged narrow review coexisted with an open critical system-level defect. Convergence within a scope says nothing about risk outside it.
...
**Example C — Anthill `node_signature`: a disclosed defect whose severity only attack-chain analysis assigned (Stages 5→6).** The observability component defined a per-node signature field intended to authenticate signals, *accepted and stored* the field, but **never verified it**, and did not even require it (`anthill/server.js`, per disposition index §3, FDI-ANT-001). The field's mere presence created the *appearance* of a stronger guarantee than the code delivered: consumers could reasonably assume signals were authenticated. Because `originating_node` was a free-form string, any party could forge attributable governance signals against any node. This defect was **not** hidden: the Anthill component review had already flagged it at P0 (arch-ant S1), the project ledger tracked it from 2026-05-15, and the Anthill specification itself disclosed it in an appendix as its largest deferred work item. What no stage had done, until cross-service trust-boundary analysis, was assign it the severity it warranted: by constructing the concrete fabrication, suppression, and framing attack chains across the producer/consumer boundary, that stage escalated a known, disclosed, deferred item to CRITICAL — and in the same pass surfaced two findings sourced to no other stage (FDI-ANT-002 suppression, FDI-RES-008 revocation freeze). **Lesson:** a field that looks like a control is not a control — but the deeper lesson is about *severity*, not detection. A defect can be openly catalogued and deferred while its true impact goes unrecognized until someone traces, at the boundary, what the consumer assumes and what an adversary could therefore do. Assigning severity is a distinct review act from finding the defect, and the boundary stage (§13.6) is where the impact of a producer/consumer trust gap becomes concrete.
...
Three findings are worth drawing out. First, **the boundary is where the impact becomes concrete.** The case's only CRITICAL, and its most consequential deployment defect (the absent mirror protocol), were both *boundary* phenomena — a field trusted-but-unverified, and a capability claimed-but-unbuilt. Component review had noted both, but at component-level weight: the `node_signature` defect sat as a disclosed, deferred P0 until cross-service attack chains made its CRITICAL impact concrete, and the mirror's impossibility was demonstrated only under attempted deployment — while two boundary findings (FDI-ANT-002, FDI-RES-008) had no other source at all. If the method has one transferable lesson, it is that component review and consistency review, however disciplined and however many rounds, do not substitute for cross-service and second-implementer review — sometimes for detection, and sometimes for recognizing how bad a known defect actually is.
...
During preparation of the stage-derivation table (§11.1), the evidence discipline of Stage 10 applied to this paper's own headline example found it overstated: the `node_signature` defect had been disclosed in the Anthill component review and specification before the cross-service stage, which assigned its severity rather than discovering it. We corrected the claim (§11, §17 Example C, §13.6) rather than remove the example. That the method surfaced its own author's overclaim, against the corpus, is a small but direct instance of the discipline the paper advocates — and a reminder that the same overclaiming pressure the method is built to catch operates on those who apply it.
