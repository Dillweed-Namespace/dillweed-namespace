...
COMPONENT="anthill"
DISPLAY="Dillweed Anthill™"
VERSION="0.1.5"
PORT=9476
INSTALL_DIR="/usr/local/dillweed/anthill/dillweed-anthill"
PLIST_LABEL="com.dillweed.anthill"
PLIST_PATH="$HOME/Library/LaunchAgents/${PLIST_LABEL}.plist"
KEYCHAIN_SERVICE="dillweed-anthill"
KEYCHAIN_ACCOUNT="anthill-admin"
...
if [ "${1}" = "--uninstall" ]; then
  ...
  warn "Signal log and database preserved. Remove manually if desired:"
  warn "  sudo rm -rf /usr/local/dillweed/anthill"
  echo ""
  exit 0
fi
...
echo "  │  dillweed.com/anthill-spec.html             │"
echo "  │  Installer                                  │"
echo "  └─────────────────────────────────────────────┘"
echo ""
...
if [ ! -d "/usr/local/dillweed/anthill" ]; then
  sudo mkdir -p /usr/local/dillweed/anthill
  sudo chown "$(whoami):wheel" /usr/local/dillweed/anthill
  ok "Created /usr/local/dillweed/anthill"
fi
...
# Preserve database and signal log if upgrading
DB_BACKUP=""
LOG_BACKUP=""
if [ -f "$INSTALL_DIR/data/anthill.db" ]; then
  DB_BACKUP=$(mktemp -d)
  cp "$INSTALL_DIR/data/anthill.db" "$DB_BACKUP/"
  info "Preserved existing signal database"
fi
if [ -f "$INSTALL_DIR/logs/signals.log" ]; then
  LOG_BACKUP=$(mktemp -d)
  cp "$INSTALL_DIR/logs/signals.log" "$LOG_BACKUP/"
  info "Preserved existing immutable signal log"
fi

mkdir -p "$INSTALL_DIR/data" "$INSTALL_DIR/logs"

for f in server.js setup.js start.sh test.sh install.sh package.json \
          LICENSE NOTICE README.md .env.example; do
  if [ -f "$SRC_DIR/$f" ]; then
...
if [ -n "$DB_BACKUP" ] && [ -f "$DB_BACKUP/anthill.db" ]; then
  cp "$DB_BACKUP/anthill.db" "$INSTALL_DIR/data/anthill.db"
  ok "Restored signal database"
  rm -rf "$DB_BACKUP"
fi
if [ -n "$LOG_BACKUP" ] && [ -f "$LOG_BACKUP/signals.log" ]; then
  cp "$LOG_BACKUP/signals.log" "$INSTALL_DIR/logs/signals.log"
  ok "Restored immutable signal log"
  rm -rf "$LOG_BACKUP"
fi
...
<dict>
  ...
  <dict>
    <key>ANTHILL_PORT</key>
    <string>${PORT}</string>
    <key>ANTHILL_ADMIN_TOKEN</key>
    <string>${TOKEN}</string>
    <key>PATH</key>
  ...
  <string>/usr/local/dillweed/logs/anthill.log</string>

  <key>StandardErrorPath</key>
  <string>/usr/local/dillweed/logs/anthill-error.log</string>
</dict>
</plist>
PLIST
...
# Secure the plist — the file contains ANTHILL_ADMIN_TOKEN in its environment
...
if curl -s "http://localhost:${PORT}/health" | grep -q '"ok"'; then
  ...
  fail "  /usr/local/dillweed/logs/anthill-error.log"
  exit 1
fi
...
echo "  │                                             │"
echo "  │  Spec: dillweed.com/anthill-spec.html       │"
echo "  └─────────────────────────────────────────────┘"
echo ""
